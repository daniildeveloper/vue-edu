<template>
  <div>
    ...
    <div class="drag-container objectStages">
      <ul class="drag-list">
        <li v-for="stage in objectStages" class="drag-column" :class="{['drag-column-' + stage.name]: true}" :key="stage.name">
          <span class="drag-column-header">
            <h2>{{ stage.name }}</h2>
          </span>
          <div class="drag-options"></div>
          <ul class="drag-inner-list" ref="list" :data-status="stage.name">
            <li class="drag-item" v-for="block in getBlocks(stage.name)" :data-block-id="block.id" :key="block.id">
                <slot :name="block.id">
                    <strong>{{ block.status }}</strong>
                    <div>{{ block.id }}</div>
                </slot>
